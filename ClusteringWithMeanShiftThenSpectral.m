...
pd = pdist2(Y',Y');
GYY = exp(-0.5*pd.^2/sigma2);
D = diag(sum(GYY,2))^(-1/2); 
L = D^(-1/2)*GYY*D^(-1/2); %Graph Laplacian
[V,D] = eig(L); [d,ind]=sort(diag(D),'descend'); 
D = diag(d); V = V(:,ind);
%figure(2), clf, stem(d,'r.'), hold on, stem(diff(d),'b.'); drawnow,
dGYY = sort(abs(eig(GYY)),'descend');
...
%%%%%%
function X = meanshift(Y)
[D,N] = size(Y); covY = cov(Y');
sigma2 = 0.5^2*(1/D)*trace(covY)*(4/((2*D+1)*N))^(2/(D+4));
